...
if [ "$battpercent" -lt 9 ] || [ "$maxtemp" -gt 90000 ]; then
    ...
    type notify-send 2> /dev/null && notify-send --urgency critical --expire-time 10000 \
        "Battery low or CPU overheating!" "Battery at ${battpercent}%, CPU temp at $(( maxtemp / 1000 ))°C"
else
